...
export default function Programmers() {
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: 'royalblue', color: currentProgrammer ? 'gold' : 'royalBlue' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { changeProgrammer(dev)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
